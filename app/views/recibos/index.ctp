...
<?php
echo $alaxosForm->create('Recibo', array('controller' => 'recibos', 'url' => $this->passedArgs));
?>
...
<?php if(in_array(Credentials::get("__credentials.Login.role_id"),array(1,3))): ?>
		<span class="lbutton"><?php echo $this->Html->link(__('Agregar Nuevo', true), array('action' => 'add_super')); ?></span>
<?php endif;?>
	...
	<tr>
			<th><?php echo $this->Paginator->sort("Cod. Recibo",'codigo');?></th>
	...
	<tr>
		<td><?php    echo $this->AlaxosForm->filter_field('secuencia',array("type"=>"text"));   ?></td>
		<td>&nbsp;</td>
		<td><?php    echo $this->AlaxosForm->filter_field('nombres');    ?></td>
		<td>&nbsp;</td>
		<td>&nbsp;</td>
		<td>&nbsp;</td>
		<td>
	...
	?>
	<tr<?php echo $class;?>>
		<td><?php echo recibo_id($recibo['Recibo']); ?>&nbsp;</td>
		<td>
